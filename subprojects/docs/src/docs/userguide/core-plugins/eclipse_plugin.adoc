...
Each classpath entry can have a a list of classpath attributes, where the attributes are string key-value pairs.
...
Similarly, only test sources can reference classes from a jar file if the jar file has the `test=true` classpath attribute.
For project dependencies if the `test=true` attribute is present the the classes in the target project is visible for test sources
If the `without_test_sources=false` attribute is present then the test sources of the target project are also accessible.

The following rules apply for the test attribute declaration when generating the Eclipse classpath:

- Source sets and dependency configurations are categorized as test if their name contain the 'test' substring.
